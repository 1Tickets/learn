...
$("#transPath").click(function () {
    ...
    if ($transRepositoryId && $transRepositoryId !== ""&&$scriptType && $scriptType !=="") {
        var treeData = findTransRepTreeById($transRepositoryId,$scriptType);
...
function findTransRepTreeById(id,type) {
    ...
    $.ajax({
        type: 'GET',
        async: false,
        url: '/dataCollection/repository/findJobRepTreeById?id=' + id+"&type="+type,
        data: {},
        success: function (data) {
            if (data.success) {
